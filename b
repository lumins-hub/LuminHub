...
local function dropAll()
	for i = 1, 5 do
		...
		if frame and frame:FindFirstChild("CurrentItem") and frame.CurrentItem:FindFirstChild("Amount") then
			local amt = frame.CurrentItem.Amount.Value
			if amt = 25 then
				Drop:FireServer(25) -- Drop fixed amount 25 only if amt > 0
			end
		end
	end
end
